...
int main()
{
    ...
    int SCR_WIDTH = 1920;
    int SCR_HEIGHT = 1080;
    std::string title = "Example 5: Models";
    Window window(SCR_WIDTH, SCR_HEIGHT, title,false);

    glfwSetKeyCallback(window.window, key_callback);
    glfwSetCursorPosCallback(window.window, cursor_position_callback);
    glfwSetInputMode(window.window, GLFW_CURSOR, GLFW_CURSOR_HIDDEN);

	std::vector<ivpt> star = parse_model_into_ivpts("resources/assets/guitar/strat.obj");
    ...
    unsigned char *data = stbi_load("resources/assets/guitar/strat_green.png", &width, &height, &nrChannels, 0);
